...
void DecayStorage::EnterNotify() {
  ...
  buy_policy.Init(this, &input, std::string("input"), &fuel_tracker, throughput).Set(incommod).Start();
  sell_policy.Init(this, &tritium_storage, std::string("output"), throughput).Set(outcommod).Start();
}
