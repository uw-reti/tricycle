...
TEST_F(ReactorTest, NormalStartup) {
  ...
  cyclus::MockSim sim(cyclus::AgentSpec
          (":tricycle:Reactor"), config, simdur);
  ...
  //under these conditions, we expect the reactor to be able to startup on timestep 0
...
TEST_F(ReactorTest, FuelConstrainedStartup) {
  ...
  //Under these conditions, we expect the reactor to be able to startup on timestep 1
  //but not before then (buys 8kg of T on each timestep, and needs 8.121kg to startup).
  //NOTE: startup occurs after DRE. 
  std::vector<Cond> conds0;
  conds0.push_back(Cond("Time", "==", std::string("0")));
  QueryResult qr0 = sim.db().Query("ReactorEvents", &conds0);
...
TEST_F(ReactorTest, WrongFuelStartup) {
  ...
  QueryResult qr2 = sim.db().Query("ReactorEvents", &conds2);
  std::string value = qr2.GetVal<std::string>("Value");

  std::string expected_message = "Startup Failed: Fuel incommod not as expected. "
                                 "Expected Composition: {{10030000,1}}. Fuel Incommod "
...
TEST_F(ReactorTest, DecayInventory) {
  ...
  //We use unintuitive values for reserve inventory and startup inventory here because
  //they were the values we were originally testing with, and we had already done
...
TEST_F(ReactorTest, GetComp) {
  ...
  cyclus::Material::Ptr Li = cyclus::Material::CreateUntracked(1, enriched_lithium());
...
TEST_F(ReactorTest, ExtractHelium) {
  ...
  cyclus::toolkit::ResBuf<cyclus::Material> test_buf;
  cyclus::Material::Ptr test_mat = cyclus::Material::CreateUntracked(1.0, decayed_tritium());
...
TEST_F(ReactorTest, ExtractNoHelium) {
  ...
  cyclus::toolkit::ResBuf<cyclus::Material> test_buf;
  cyclus::Material::Ptr test_mat = cyclus::Material::CreateUntracked(1.0, tritium());
